"""
Скрипт для интеграции Dr.Web ICAPD и прокси-сервера Squid.
"""
...
def run_shell_command(command: list, title: str = ""):
    ...
    try:
        ...
        raise RuntimeError(
            f"Команда '{command[0]}' не найдена. Убедитесь, что утилита установлена и доступна в PATH."
        ) from e
    except Exception as e:
        raise RuntimeError(f"Непредвиденная ошибка при выполнении команды: {e}") from e
        ...
        if self.log_file_path != log_file:
            ...
            if self.log_file_path:
                try:
                    ...
                    sys.stderr.write(
                        f"ПРЕДУПРЕЖДЕНИЕ: Не удалось открыть лог-файл '{self.log_file_path}': {e}\n"
                    )
                    self.log_file = None
...
def check_root():
    ...
    if os.geteuid() != 0:
        logger.error(
            "Ошибка: Эта команда требует прав суперпользователя (root). Запустите ее через sudo."
        )
        sys.exit(1)
...
def check_drweb_standalone_mode():
    ...
    if is_enterprise_mode:
        logger.info(
            "Обнаружен режим централизованной защиты. "
            "Данный скрипт применит все необходимые настройки для squid, но настройки Dr.Web остануться прежними"
        )
        return False
    logger.debug(
        "Проверка режима Dr.Web: обнаружен автономный (standalone) режим. Продолжение работы."
    )
    return True
...
def check_drweb_license():
    ...
    if "no license" in output.lower():
        logger.error(
            "Отсутствует лицензия Dr.Web. Активируйте продукт Dr.Web и попробуйте снова."
        )
        sys.exit(1)
...
def check_squid_version(args):
    ...
    output = run_shell_command(
        ["squid", "-v"], "Проверка совместимости установленной версии Squid с Dr.Web"
    )

    # Берет первую строку вывода и ищет номер версии
    version_line = output.split("\n")[0]
    for i, ch in enumerate(version_line):
        if ch.isdigit():
            version = version_line[i:].strip()
            break
    ...
    if "--enable-icap-client" not in output:
        logger.error(
            "Ошибка: Установленная версия Squid была скомпилирона без флага --enable-icap-client."
        )
        sys.exit(1)
    ...
    if args.command == "setup":
        if args.with_ssl:
            if "--with-openssl" not in output or "--enable-ssl-crtd" not in output:
                logger.error(
                    "Ошибка: Установленная версия Squid была скомпилирона без поддержки разбора HTTPS. Перекомпилируйте squid с флагами --with-openssl и --enable-ssl-crtd."
                )
                sys.exit(1)
    try:
        ...
        logger.debug(f"Минорная версия squid: {minor_version}")
...
def find_squid_config_dir(args) -> Path:
    ...
    if args.squid_config_dir:
        ...
        if config_dir.is_dir():
            logger.debug(
                f"Используется директория Squid, указанная вручную: {config_dir}"
            )
            return config_dir
        else:
            raise FileNotFoundError(
                f"Указанная директория Squid не найдена: {config_dir}"
            )
    ...
    raise FileNotFoundError(
        "Не удалось автоматически определить конфигурационную директорию Squid."
    )
...
def check_squid_syntax():
    ...
    try:
        output = run_shell_command(
            ["squid", "-k", "parse"], title="Проверка синтаксиса конфигурации Squid"
        )
        ...
        logger.error(
            "КРИТИЧЕСКАЯ ОШИБКА: Проверка конфигурации Squid провалилась!",
            to_stderr=False,
        )
        logger.error(
            "Служба Squid не будет перезапущена, чтобы избежать сбоя.", to_stderr=False
        )
        raise e
...
def _replace_or_add_block(
    ...
    if pattern.search(content):
        logger.debug(
            f"Найден существующий блок конфигурации '{block_name}'. Заменяем его."
        )
        return pattern.sub(new_block, content)

    logger.debug(
        f"Блок конфигурации '{block_name}' не найден. Добавляем новый в конец файла."
    )
...
def build_http_port_line(
    squid_port: int, cert_dir: Path, use_tls_cert: bool = True
) -> str:
    """
        ...
        if ssl_lines:
            ...
            try:
                ...
                if result.stderr.strip():
                    logger.debug(
                        "Настройки порта выдали ошибку. Пробуем другую конфигурацию..."
                    )
                    # Заменяем tls-cert на cert в строке http_port внутри блока
                    cert_dir = filepath.parent
                    fallback_line = build_http_port_line(
                        args.squid_port, cert_dir, use_tls_cert=False
                    )
                    # Находим и заменяем строку внутри блока
                    pattern = r"^http_port.*tls-cert.*\n$"
                    content = re.sub(
                        pattern, fallback_line + "\n", content, flags=re.MULTILINE
                    )
                ...
                logger.error(
                    "Команда 'squid' не найдена. Проверка конфигурации пропущена."
                )
...
def get_ssl_lines():
    ...
    """
    # Список с возможными путями к утилите для работы с SSL сертификатами
    cmds = [
        "/usr/sbin/ssl_crtd",
...
def handle_setup(args, squid_config_dir: Path, version: int, mode: bool):
    ...
    if not main_cf_path.exists():
        raise FileNotFoundError(
            f"Основной файл конфигурации Squid не найден: {main_cf_path}"
        )
...
def add_certificate_to_trusted(cert_path):
    ...
    try:
            ...
            for cmd in cmds:
                try:
                    ...
                    if cmd == "update-ca-trust":
                        run_shell_command(
                            ["cp", str(cert_path), "/etc/pki/ca-trust/source/anchors/"]
                        )
                        run_shell_command([cmd])
                        return
                except Exception:
        ...
        logger.warning(
            f"Не получилось добавить созданный сертификат в список доверенных. \nПожалуйста сделайте это сами. Путь к сертификату {cert_path}"
        )
        return
    ...
    try:
        ...
        run_shell_command(
            [
                ...
                "v3_ca",
                "-keyout",
                str(ssl_dir_path / "squid.key"),
                "-out",
                str(cert_path),
                "-subj",
                "/C=RU/ST=SPB/L=SPB/O=Dr.Web/OU=IT/CN=proxy.drweb.com",
            ]
        )
        ...
        logger.warning(
            "Не получилось создать SSL-сертификат. Пожалуйста сделайте это самостоятельно."
        )
        return
...
def prepare_ssl_db():
    ...
    try:
        ...
        if cmd:
            ...
            run_shell_command(
                ["chown", "-R", f"{user}:{user}", "/usr/local/squid/var/lib/"]
            )
            run_shell_command(
                [cmd[0], "-c", "-s", "/usr/local/squid/var/lib/ssl_db", "-M", "20MB"]
            )
            run_shell_command(["chown", "-R", f"{user}:{user}", "/usr/local/squid"])
    except Exception:
        logger.warning(
            "Не получилось подготовить базу данных SSL сертификатов. Пожалуйста подготовьте ее самостоятельно."
        )
        return
...
def create_backup(filepath: Path):
    ...
    try:
        ...
        logger.debug(
            f"[*] Создана резервная копия '{filepath.name}' -> '{backup_path.name}'"
        )
    except Exception as e:
        logger.warning(
            f"ПРЕДУПРЕЖДЕНИЕ: Не удалось создать резервную копию для '{filepath}': {e}"
        )
...
def remove_block(
    ...
    """
    pattern = re.compile(
        rf"\s*?{re.escape(header)}.*?{re.escape(footer)}\s*?", re.DOTALL
    )
    if pattern.search(content):
    ...
    logger.info(
        f"[*] Блок конфигурации '{block_name}' не найден. Действий не требуется."
    )
...
def remove_squid_config_block(filepath: Path, with_ssl: bool) -> None:
    ...
    content, removed_main = remove_block(content, BLOCK_HEADER, BLOCK_FOOTER, "icap")

    # 2. Если требуется, удаляем SSL-блоки
    if with_ssl:
        # Удаляем блок ssl_bump
        content, _ = remove_block(
            content, SSL_BLOCK_HEADER, SSL_BLOCK_FOOTER, "ssl_bump"
        )

        # Удаляем блок http_port (с настройками порта)
        content, _ = remove_block(
            content, SSL_PORT_HEADER, SSL_PORT_FOOTER, "http_port"
        )
    ...
    if removed_main or (
        with_ssl and content != filepath.read_text(encoding="utf-8", errors="ignore")
    ):
        filepath.write_text(content, encoding="utf-8")
        logger.success(f"[+] Конфигурация Dr.Web успешно удалена из '{filepath.name}'.")
    else:
        logger.info(
            f"[*] В файле '{filepath.name}' не найдено блоков Dr.Web для удаления."
        )
...
def handle_remove(args, squid_config_dir: Path):
    ...
    if not args.yes:
        confirm = input(
            "--> Вы уверены, что хотите удалить конфигурацию Dr.Web из Squid? [y/N]: "
        ).lower()
        if confirm != "y":
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        "  --squid-port        Порт для Squid сокета (по умолч.: 3128)\n"
    ...
    base_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Сохранять весь вывод в файл журнала.",
    )
    base_parser.add_argument(
        "-d",
        "--debug",
    ...
    parser_icapd = subparsers.add_parser(
        "setup", parents=[base_parser], help="Настройка интеграции через ICAPD."
    )
    parser_icapd.add_argument(
        "--icapd-host",
        dest="icapd_host",
        type=str,
        default="127.0.0.1",
    ...
    logger.reconfigure(
        debug_mode=args.debug, color_enabled=not args.no_color, log_file=args.log_file
    )
    try:
        check_root()
        check_drweb_license()
        ...
        if "freebsd" in run_shell_command(["uname", "-a"]).lower():
            ...
            run_shell_command(
                ["systemctl", "restart", "squid"], title="Перезапуск Squid"
            )
        ...
        if args.command.startswith("setup"):
            if "freebsd" in run_shell_command(["uname", "-a"]).lower():
                run_shell_command(
                    ...
                    title="Перезапуск Dr.Web ConfigD",
                )
            else:
                run_shell_command(
